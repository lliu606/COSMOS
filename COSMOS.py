...
def soap_with_muon_gpt(params: List[Tensor],
            grads: List[Tensor],
...
def soap_with_muon_llama(params: List[Tensor],
          grads: List[Tensor],
          exp_avgs: List[Tensor],
          exp_avg_sqs: List[Tensor],
          exp_avgs_GG: List[Tensor],
    ...
    for i, param in enumerate(params):
        ...
        if len(param.size()) == 2 and param.size(0) <= 10000:
            ...
            if step == 1:
                ...
                U, _, _ = torch.linalg.svd(W, full_matrices=False)
                exp_avg_p.data = U[:, :exp_avg_gg.size(0)]
                exp_avg_gg = torch.matmul(torch.matmul(exp_avg_p.T, grad.T), torch.matmul(grad, exp_avg_p)) * (1 - beta3)

            else:
                t = exp_avg_p.detach().clone().T
                exp_avg_p = beta3 * torch.matmul(exp_avg_p, exp_avg_gg) + (1 - beta3) * torch.matmul(grad.T,torch.matmul(grad,exp_avg_p))
                exp_avg_p, _ = torch.linalg.qr(exp_avg_p, mode='reduced')
                t = torch.matmul(t, exp_avg_p)
                exp_avg_gg = beta3 * torch.matmul(t.T, torch.matmul(exp_avg_gg, t)) + \
                             (1 - beta3) * torch.matmul(torch.matmul(grad, exp_avg_p).T, torch.matmul(grad, exp_avg_p))
...
class COSMOS_for_llama(Optimizer):
    ...
    def __init__(self, params, lr=1e-3, betas=(0.9, 0.98, 0.98), eps=1e-8, lr_ratio=0.1, rank=64,
                 weight_decay=0, amsgrad=False, *, maximize: bool = False):
        if not 0.0 <= lr:
            raise ValueError("Invalid learning rate: {}".format(lr))
        if not 0.0 <= eps:
            ...
            for p in group['params']:
                if p.grad is not None:
                    ...
                    if len(state) == 0:
                        ...
                        if len(p.size()) == 2 and p.size(0) <= 10000:
                            state['exp_avg_GG'] = torch.zeros(self.rank, self.rank, dtype=p.data.dtype,
                                                              device=p.data.device)
                            state['exp_avg_P'] = torch.zeros(p.size(1), self.rank, dtype=p.data.dtype,
                                                             device=p.data.device)
                            state['exp_avg_sq'] = torch.zeros(p.size(0), self.rank, dtype=p.data.dtype,
                                                              device=p.data.device)
